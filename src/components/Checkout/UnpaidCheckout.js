...
export default class UnpaidCheckout extends Component {
    state = {
        user: [],
        isLoggedIn: false,
        checkouts: [],
    ...
    async componentDidMount(){
        ...
        return fetch(BASE_URL+'user/unpaid?token='+this.state.user.token).then((response) => response.json()).then((res) => {
            if(res.isError){
                alert(res.message);
            }else if(res.isFound){
    ...
    gotoCheckout = (checkout_id) => {
        ...
        this.props.navigation.navigate('UnPaidCKProducts', {ck_id: checkout_id});
    }
